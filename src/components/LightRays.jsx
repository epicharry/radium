...
export default function LightRays({
  ...
  const mouseThrottleIdRef = useRef(null)
  const resizeTimeoutRef = useRef(null)
  const observerRef = useRef(null)

  const devicePixelRatio = Math.min(window.devicePixelRatio || 1, 2)
  const rgbColor = hexToRgb(raysColor)
  const pulsatingValue = pulsating ? 1.0 : 0.0

  const debouncedUpdatePlacement = (updateFn) => {
  ...
  const handleMouseMove = (e) => {
    ...
    if (mouseThrottleIdRef.current) return

    mouseThrottleIdRef.current = requestAnimationFrame(() => {
      ...
      mouseThrottleIdRef.current = null
    })
  }
  ...
  const initializeWebGL = async () => {
    ...
    try {
      ...
      const uniforms = {
        ...
        pulsating: { value: pulsatingValue },
  ...
  const cleanup = () => {
    ...
    if (mouseThrottleIdRef.current) {
      cancelAnimationFrame(mouseThrottleIdRef.current)
      mouseThrottleIdRef.current = null
    }
  ...
  useEffect(() => {
    ...
    u.pulsating.value = pulsatingValue
  ...
  return (
    <div
      ref={containerRef}
      className={`w-full h-full fixed top-0 left-0 pointer-events-none z-[3] overflow-hidden ${className}`}
    />
  )
}
